...
for K in list(reversed(K_vals)):
  ...
  plt.plot(N_vals, y_vals_P, marker=markers[K_vals.index(K)], color=colors[0], label= "P, K=%d, d=%d" % (K, fixed_d))
  plt.plot(N_vals, y_vals_S, marker=markers[K_vals.index(K)], color=colors[1], label= "S, K=%d, d=%d" % (K, fixed_d))

plt.gca().xaxis.set_ticks(N_vals)
plt.title("Varying N for fixed K and d")
plt.xlabel("Bugdet N")
...
for N in list(reversed(N_vals)):
  ...
  for line in lines:
    if line[0] == N and line[2] == fixed_d:
      y_vals_P.append(line[3])
      y_vals_S.append(line[4])
  plt.plot(K_vals, y_vals_P, marker=markers[N_vals.index(N)], color=colors[0], label= "P, N=%d, d=%d" % (N, fixed_d))
  plt.plot(K_vals, y_vals_S, marker=markers[N_vals.index(N)], color=colors[1], label= "S, N=%d, d=%d" % (N, fixed_d))
  
plt.gca().xaxis.set_ticks(K_vals)
plt.title("Varying K for fixed N and d")
...
for fixed_N in [N_vals[2], N_vals[0]]:
  for fixed_K in [K_vals[2], K_vals[0]]:
    ...
    plt.plot(d_vals, y_vals_P, marker=list(reversed(markers))[i], color=colors[0], label= "P, N=%d, K=%d" % (fixed_N, fixed_K))
    plt.plot(d_vals, y_vals_S, marker=list(reversed(markers))[i], color=colors[1], label= "S, N=%d, K=%d" % (fixed_N, fixed_K))
    i += 1
  
plt.gca().xaxis.set_ticks(d_vals)
plt.title("Varying d for fixed N and K")
